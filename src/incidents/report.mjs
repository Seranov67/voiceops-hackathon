...
export function buildReport({runId,result,service='nginx'}) {
  ...
  return {schemaVersion:'1.0',runId,source:result.source,service,host:'synthetic-host',status,severity:'unknown',summary,probable_cause:probableCause,evidence:records.length?records:[],recommended_actions:status==='incident'?['Inspect upstream service status and application logs using read-only tools']:[],confidence,limitations:['Synthetic fixture data','Business impact was not measured']};
}
