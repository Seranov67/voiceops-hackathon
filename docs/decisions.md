...
The automated live capture gate is now complete. Two browser exports were merged with a deterministic selector that requires clean termination, successful tool execution, the expected scenario outcome, and all schema, provenance, and policy validations. The committed formal matrix contains 15/15 passing runs, with three selected per scenario. Average server tool latency is 8.85 ms and p95 is 10.6 ms; conversational latency was not measured by this metric. See `artifacts/evaluations/live-formal-matrix.json`.
