...
test('voice endpoint fails safely without a key',async()=>{const id=await session();const response=await fetch(base+'/api/voice-token',{method:'POST',headers:{'x-voiceops-session':id}});assert.equal(response.status,503);assert.doesNotMatch(JSON.stringify(await response.json()),/ASSEMBLYAI_API_KEY/);});
